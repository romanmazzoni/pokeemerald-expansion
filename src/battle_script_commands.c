...
END:
    ...
    if (gBattleWeather & B_WEATHER_STRONG_WINDS)
    {
        if ((gBattleMons[gBattlerTarget].type1 == TYPE_FLYING
         && GetTypeModifier(moveType, gBattleMons[gBattlerTarget].type1, 0) >= UQ_4_12(2.0))
         || (gBattleMons[gBattlerTarget].type2 == TYPE_FLYING
         && GetTypeModifier(moveType, gBattleMons[gBattlerTarget].type2, 0) >= UQ_4_12(2.0))
         || (gBattleMons[gBattlerTarget].type3 == TYPE_FLYING
         && GetTypeModifier(moveType, gBattleMons[gBattlerTarget].type3, 0) >= UQ_4_12(2.0)))
        {
            gBattlerAbility = gBattlerTarget;
            BattleScriptPushCursor();
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            #else
                *exp = calculatedExp;
                gExpShareExp = calculatedExp * 2 / 3;
                if (gExpShareExp == 0)
                    gExpShareExp = 1;
            #endif
...
static void Cmd_settypetorandomresistance(void)
{
    ...
    else
    {
        ...
        {
            switch (GetTypeModifier(hitByType, i, 0))
            {
            case UQ_4_12(0):
            case UQ_4_12(0.5):
...
static void Cmd_presentdamagecalculation(void)
{
    ...
    if (gSpecialStatuses[gBattlerAttacker].parentalBondState != PARENTAL_BOND_2ND_HIT)
    {
        if (rand < 102)
        {
            gBattleStruct->presentBasePower = 55;
        }
        else if (rand < 178)
        {
            gBattleStruct->presentBasePower = 85;
        }
        else if (rand < 204)
        {
