    ...
    while (bits != 0x3F) // All mons were checked.
    {
        ...
        for (i = firstId; i < lastId; i++)
        {
            if (!(gBitTable[i] & invalidMons) && !(gBitTable[i] & bits))
            {
                ...
                MulModifier(&typeEffectiveness, (GetTypeModifier(atkType1, defType1, 0)));
                if (atkType2 != atkType1)
                    MulModifier(&typeEffectiveness, (GetTypeModifier(atkType2, defType1, 0)));
                if (defType2 != defType1)
                {
                    MulModifier(&typeEffectiveness, (GetTypeModifier(atkType1, defType2, 0)));
                    if (atkType2 != atkType1)
                        MulModifier(&typeEffectiveness, (GetTypeModifier(atkType2, defType2, 0)));
                }
